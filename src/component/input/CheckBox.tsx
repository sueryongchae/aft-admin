...
const CheckBox = ({ _type, _name = '', _defaultChecked = false, _handleChange = () => {} }: Props) => {
  ...
  return (
    <div className="flex">
      {_type === 1 && (
        <label css={styles1}>
          <input type="checkbox" onChange={_handleChange} defaultChecked={_defaultChecked} />
          <div></div>
        </label>
      )}
      ...
      {_type === 2 && (
        <label css={styles2}>
          <input type="checkbox" onChange={changeStatus} defaultChecked={_defaultChecked} />
          <div>{_name}</div>
        </label>
      )}

      {_type === 3 && (
        <label css={styles3({ status })}>
          <input type="checkbox" onChange={changeStatus} defaultChecked={_defaultChecked} />
          <div>{_name}</div>
        </label>
      )}
...
const styles3 = ({ status }: { status: boolean }) => [
  tw`
  ...
  `,
  css`
    background-color: ${status ? 'var(--White)' : 'var(--Gray0)'};
    box-shadow: 0 0 0 1px ${status ? 'var(--Blue6)' : 'var(--Gray3)'} inset;
    color: ${status ? 'var(--Blue6)' : 'var(--Gray7)'};
